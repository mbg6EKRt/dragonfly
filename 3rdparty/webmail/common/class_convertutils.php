<?php
	...
	{
		/**
		 * Gets the name of internal library, used for string conversion
		 * @return string
		 */
		function GetEncodingLib()
		{
			if (extension_loaded('mbstring') && USE_MBSTRING)
			{
				return 'mbstring';
			}
			elseif (extension_loaded('iconv') && USE_ICONV)
			{
				return 'iconv';
			}
		...
		/**
		 * Converts encoding using iconv or mbstring or our custom convertor
		 * @param string $str
		...
		function ClassIconv($from_encoding, $to_encoding, $str)
		{
			$iconv_encodings_array = array('ascii',
				...
				'euc-cn', 'hz', 'gbk', 'gb18030', 'euc-tw', 'big5', 'big5-hkscs', 
		...
		{
			if (ConvertUtils::IsLatin($str) && $from_encoding != CPAGE_UTF7_Imap) return $str;
...
//			}
		...
		function EncodeString($str, $toCharset)
		{
			...
			//$factor = (MIMEConst_DefaultQB == 'Q') ? 0.3 : 0.55;
...
*/
		...
		function GetTimeFromString($strDate)
		{
			...
			if (preg_match($datePattern, $strDate, $matches))
			{
				...
				$dt = ConvertUtils::ApplyOffsetForDate($dt, $matches[10], $zone);
			}
		...
		{
			return str_replace(array('"', '/', '\\', '*','?', '<', '>', '|', ':'), '', $strFileName);
		}
