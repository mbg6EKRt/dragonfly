...
$Config['UserFilesPath'] = FCKEditor_GetRoot('url') ;

// Fill the following value it you prefer to specify the absolute path for the
// user files directory. Useful if you are using a virtual directory, symbolic
// link or alias. Examples: 'C:\\MySite\\userfiles\\' or '/root/mysite/userfiles/'.
// Attention: The above 'UserFilesPath' must point to the same directory.
$Config['UserFilesAbsolutePath'] = FCKEditor_GetRoot() ;
...
/* The following was modified by Robin Stoker for WebBuilder.
   16 March 2011.
   Added function below to dynamically get root path
*/
...
//-----------------------------------------------------------------------------
function FCKEditor_GetRoot($What = 'path', $Path = 'media/common')
{
	...
	switch ($What) {
		...
		case 'PATH':
			$RetStr = '';
			$ThisPath = dirname($_SERVER["SCRIPT_FILENAME"]);
			if (ereg('\\', $ThisPath)) {

				// Windows type path (eg: c:\httpdocs\)
			...
			if (ereg(':', $Parts[0])) { $RetStr = $Parts[0]; $Start = 1; }
			else { $Start = 0; }//else

			for ($i = $Start; $i < $Count - 6; $i++) {
		...
		case 'URL':
			...
			if ($Port == '443') $Server = 'https://'.$Server;
			else if ($Port != '80') $Server = 'http://'.$Server.':'.$Port;
			else $Server = 'http://'.$Server;

			$Folder = '';
			$FolderArr = explode('/', $_SERVER["SCRIPT_NAME"]);
			$FCount = count($FolderArr);

			for ($i = 0; $i < $FCount - 7; $i++) {
				if (!empty($FolderArr[$i])) $Folder = $Folder.'/'.$FolderArr[$i];
			}//for
