...
CViewMessageScreen.prototype = {
	...
	{
		if (historyArgs.ScreenId = this.Id)
			if (null != historyArgs.MsgUid && null != historyArgs.MsgCharset && null != historyArgs.MsgParts) {
				GetMessageHandler(historyArgs.MsgUid, historyArgs.MsgParts, historyArgs.MsgCharset);
			}
	},
	...
	GetNextMessage: function ()
	{
		...
		if (isLastMessage) {
			...
			GetHandler(TYPE_MESSAGES_LIST, { Page: this._messagesList.Page+1 }, [], '' );
		} else if ((this.MessageIndex + 1) < this._messagesList.List.length) {
	...
	Fill: function (msg)
	{
		var isSameMsg = this._msgObj == null || msg.Uid == this._msgObj.Uid && msg.Charset == this._msgObj.Charset;
		var isRefillMsg = !this._needHeaders || !isSameMsg;
		this._msgObj = msg;
		...
		if (this.MessageIndex != -1)
		{
			...
			if (isRefillMsg)
			{
				...
				this._printButton.onclick = this.CreatePrintLinkFunc(msg.PrintLink)
			}
			this.ResizeBody();
		}
	},
	...
	CreatePrintLinkFunc: function (link)
	{
		var obj = this;
		return function () {
			...
			var win = window.open(link, 'Popup', 'toolbar=yes,status=no,scrollbars=yes,resizable=yes,width=' + width + ',height=' + height + ',top=' + top + ',left=' + left);
		}
	},
	...
	Build: function (container, accountsBar, PopupMenus, settings)
	{
		...
		a.onclick = function () {
			if (obj._showHeaders) {
				...
				GetMessageHandler(obj.msgUid, [PART_MESSAGE_FULL_HEADERS], obj.Charset);
			}
			return false;
		}
		...
		a.onclick = function () {
			...
			GetMessageHandler(obj.msgUid, [part], obj.Charset);
			return false;
		}
	...
	BuildToolBar: function (PopupMenus)
	{
		...
		item = toolbar.AddItem(TOOLBAR_DELETE, function () {
			if (confirm(Lang.ConfirmAreYouSure)) {
				...
				operation.Messages.setVal('Inbox', {IdArray: [{Id: obj.msgId, Uid: HtmlDecode(obj.msgUid)}]});
