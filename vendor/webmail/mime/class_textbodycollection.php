	...
	class TextBodyCollection
	{
		...
		function AddToCollection(&$mimePart)
		{
			if ($mimePart->IsMimePartTextBody())
			{
				...
				$log->WriteLine('AddToCollection::');
				$log->WriteLine("\$mimePart:\r\n".ConvertUtils::PrintVarDump($mimePart));
				...
				if (strpos($contentType, MIMETypeConst_TextPlain) !== false || $mimePart->Headers->Count() ==0)
				{
					$this->PlainTextBodyPart .= trim($mimePart->GetBody());
					$log->WriteLine('add plain');
				}
				elseif (strpos($contentType, MIMETypeConst_TextHtml) !== false)
				{
					$preStr = '/(<meta\s.*)(charset\s?=)([^"\'>\s]*)/i';
					$this->HtmlTextBodyPart .= trim(preg_replace($preStr, '$1$2'.$GLOBALS[MailOutputCharset], $mimePart->GetBody()));
					$log->WriteLine('add html');					
				}
				else
				{
					$log->WriteLine('dont add');
				}
			}
		}
		...
		{
			$pars = &new convertHtml($this->HtmlTextBodyPart, true);
